...
-- Helpers setY(y?) and call(fn) are provided.
...
-- marker helpers for building steps
...
local function normalizeStep(entry)
    ...
    if t == "table" then
        ...
        elseif typeof(entry) == "Vector3" then
            ...
            -- table but not recognized: maybe user wrote { point1 = Vector3.new(...) } -> try extract Vector3 inside table
...
local function buildStepsFromSpec(spec)
    ...
    -- (important: recommend author to use array style to guarantee ordering)
    local out = {}
    for i=1,#spec do
