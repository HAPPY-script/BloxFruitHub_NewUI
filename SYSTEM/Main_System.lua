...
do
    ...
    local function setHRPInstant(hrp, pos)
    	pcall(function()
    ...
    local function Lunge(targetPos)
    	...
    	local delta = finalPos - startPos
    	local dist = delta.Magnitude
    
    	if dist < 0.5 then
    		setHRPInstant(hrp, finalPos)
    		moveDone = true
    		return true
    	end
    ...
    --// HIGH-Y LUNGE FOR FAR FARM TRAVEL
    -- TP Y = 3000 tuyệt đối
    -- Chỉ lunge X/Z
    -- Xong thì TP xuống Y thật của target
    local function lungeSetY(targetPos)
    	if typeof(targetPos) ~= "Vector3" then
    		return false
    	end
    	...
    	local startMovePos = Vector3.new(startPos.X, HIGH_Y, startPos.Z)
    	local targetMovePos = Vector3.new(targetPos.X, HIGH_Y, targetPos.Z)
    	local finalPos = targetPos
    
    	setHRPInstant(hrp, startMovePos)
    ...
    local function idle(zone)
    	...
    	task.spawn(function()
    		...
    		while running and farmCenter do
    			...
    			-- xa hơn distanceLimit thì mới dùng kiểu high-Y
    			if distToCenter > distanceLimit then
    				lungeSetY(farmCenter)
    				waitMoveDone(10)
    				task.wait(0.1)
    				continue
    
    			-- trong phạm vi gần hơn thì đi bình thường
    ...
    local function updateTarget()
    	if not running or not farmReady then
    		return
    	end
    ...
    local function ON_ENABLE()
    	...
    	if zone then
    		...
    		-- khởi đầu luôn đi tới farm bằng kiểu high-Y
    		lungeSetY(farmCenter)
    		waitMoveDone(10)
    		task.wait(0.05)
