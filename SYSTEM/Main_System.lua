...
do
    ...
    -- TP lên HIGH_Y
    -- Lunge chỉ theo X/Z
    -- Sau đó TP xuống Y thật
    local function lungetofarmpos(targetPos)
        ...
        local startMovePos = Vector3.new(
            startPos.X,
            HIGH_Y,
            startPos.Z
        )
    
        local targetMovePos = Vector3.new(
            targetPos.X,
            HIGH_Y,
            targetPos.Z
        )
    ...
    local function ON_ENABLE()
    	...
    	if zone then
    		...
    		-- đi tới farm trước, dùng kiểu TP Y=3000
    		lungetofarmpos(farmCenter)
    		waitMoveDone(10)
    		task.wait(0.05)
